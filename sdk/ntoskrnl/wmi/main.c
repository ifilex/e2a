...
int main(int argc, char **argv) {
    ...
    for (i=1;i<argc;i++) {
        if (!strcmp(argv[i], "-root") && i+1<argc) {
            ...
            zip = argv[i+1];
#else
            printf("Wine wasn't compiled with zlib support");
#endif
            i++;
    ...
    if (startProcess(workingDir, argc, (const char**)argv, envc, ppenv, userId, groupId, effectiveUserId, effectiveGroupId)) {
        ...
        while (getProcessCount()>0) {
            ...
            if (lastTitleUpdate+5000 < t) {
                ...
                sprintf(tmp, "WineBOX %d MIPS (%d MHz) RAM %d/%dMB opCache %dMB", getMIPS(), getMHz(), (getPageCount()-getFreePageCount())/256, getPageCount()/256, allocatedOpMemory/1024/1024);
#else
                sprintf(tmp, "WineBOX %d MIPS RAM %d/%dMB opCache %dMB", getMIPS(), (getPageCount()-getFreePageCount())/256, getPageCount()/256, allocatedOpMemory/1024/1024);
#endif
                fbSetCaption(tmp, "WineBOX");
