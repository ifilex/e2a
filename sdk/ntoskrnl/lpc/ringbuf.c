...
size_t
ringbuf_buffer_size(const struct ringbuf* rb)
{
    return rb->size;
}
...
ringbuf_end(const struct ringbuf* rb)
{
    return rb->buf + ringbuf_buffer_size(rb);
}

size_t
ringbuf_bytes_free(const struct ringbuf* rb)
{
    if (rb->head >= rb->tail)
...
size_t
ringbuf_bytes_used(const struct ringbuf* rb)
{
    return ringbuf_capacity(rb) - ringbuf_bytes_free(rb);
}

int
ringbuf_is_full(const struct ringbuf* rb)
{
    return ringbuf_bytes_free(rb) == 0;
}

int
ringbuf_is_empty(const struct ringbuf* rb)
{
    return ringbuf_bytes_free(rb) == ringbuf_capacity(rb);
}

const void *
ringbuf_tail(const struct ringbuf* rb)
{
    return rb->tail;
}

const void *
ringbuf_head(const struct ringbuf* rb)
{
    return rb->head;
}
...
*/
static uint8_t *
ringbuf_nextp(struct ringbuf* rb, const uint8_t *p)
{
    /*
...
size_t
ringbuf_findchr(const struct ringbuf* rb, int c, size_t offset)
{
...
size_t
ringbuf_memset(struct ringbuf* dst, int c, size_t len)
{
...
void *
ringbuf_memcpy_into(struct ringbuf* dst, const void *src, size_t count)
{
...
void *
ringbuf_memcpy_from(void *dst, struct ringbuf* src, size_t count)
{
...
void *
ringbuf_copy(struct ringbuf* dst, struct ringbuf* src, size_t count)
{
