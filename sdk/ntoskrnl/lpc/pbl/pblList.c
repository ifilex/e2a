...
)
{
    ...
    ( (PblLinkedList *)newList )->collection.compare
            = list->collection.compare;
...
)
{
    ...
    {
        unsigned char * to =
                (unsigned char*)&( pblList->pointerArray[ pblList->collection.size
                        - elementsToRemove ] );
...
)
{
    ...
    qsort( list->pointerArray, (size_t)list->collection.size,
           (size_t)sizeof(void*), ( compare ? compare
                   : list->collection.compare ? list->collection.compare
