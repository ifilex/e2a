...
// ARPL Ew,Rw
void decode063(struct DecodeData* data) {
    U8 rm = FETCH8(data);
    if (rm>=0xC0) {
        data->op->func = arplr16r16;
        data->op->r1 = G(rm);
        data->op->r2 = E(rm);
        LOG_OP2("ARPL", R16(data->op->r1),R16(data->op->r2));
    } else {
        if (data->ea16) {
            data->op->func = arple16r16_16;
            data->op->r1 = G(rm);
            decodeEa16(data, rm);
            LOG_OP2("ARPL", R16(data->op->r1),M16(data, rm, data->op));
        } else {
            data->op->func = arple16r16_32;
            data->op->r1 = G(rm);
            decodeEa32(data, rm);
            LOG_OP2("ARPL", R16(data->op->r1),M16(data, rm, data->op));
        }
    }
    LOG_OP2("ARPL", RW(data->op->r1), M16(data, rm, data->op));
    NEXT_OP(data);
}
...
DECODER decoder[1024] = {
    ...
    decode060, decode061, decode062, decode063, decode064, decode065, decode066, decode067,
