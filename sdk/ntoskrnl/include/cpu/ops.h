...
void OPCALL arplr16r16(struct CPU* cpu, struct Op* op) {   
    cpu->reg[op->r1].u16 = CPU_ARPL(cpu, cpu->reg[op->r1].u16,cpu->reg[op->r2].u16);                
    NEXT();
}

void OPCALL arple16r16_16(struct CPU* cpu, struct Op* op) {  
    U32 eaa = eaa16(cpu, op);
    writew(cpu->thread, eaa, CPU_ARPL(cpu, readw(cpu->thread, eaa),cpu->reg[op->r1].u16));
    NEXT();
}

void OPCALL arple16r16_32(struct CPU* cpu, struct Op* op) {   
    U32 eaa = eaa32(cpu, op);
    writew(cpu->thread, eaa, CPU_ARPL(cpu, readw(cpu->thread, eaa),cpu->reg[op->r1].u16));
    NEXT();
}