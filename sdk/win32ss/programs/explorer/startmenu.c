...
{
//    LPCWSTR what="shutdown";
//    Control_RunDLLW(GetDesktopWindow(), 0, what, SW_SHOW);
//    system("control shutdown.cpl &");
    WinExec("wxbasic.exe killbox.wx &", SW_SHOWNORMAL);
}
    ...
    case WM_MENUCOMMAND:
        {
            ...
            if (item)
                ...
                run_dialog();
            else if (mii.wID == MENU_ID_SHUTDOWN)
                run_shutdown();
...
// -- SALIDA
    ...
    LoadStringW(NULL, IDS_SHUTDOWN, run_label, sizeof(run_label)/sizeof(run_label[0]));

    mii.cbSize = sizeof(mii);
    mii.fMask = MIIM_STRING|MIIM_ID;
    mii.dwTypeData = run_label;
    mii.wID = MENU_ID_SHUTDOWN;
